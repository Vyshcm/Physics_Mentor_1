...
from .forms import SignupForm,LoginForm,ForgotPasswordForm,FeedbackForm,DoubtForm, AssignmentSubmissionForm
...
def dashboard(request):
    ...
    if request.user.is_staff or request.user.is_superuser or profile.role == 'Teacher':
        ...
        # Assignments for student: Filter by standard
        assignments = Assignment.objects.filter(standard=profile.standard).order_by('-created_at')
        
        # Quizzes for student: Filter by standard
...
def feedback_view(request):
    ...
    # Calculate stats for the student feedback dashboard
    feedbacks_list = Feedback.objects.filter(student=request.user).order_by('-created_at')
    total_feedbacks = feedbacks_list.count()
    
    last_week = timezone.now() - timedelta(days=7)
    weekly_feedbacks = feedbacks_list.filter(created_at__gte=last_week).count()
    
    latest_feedback = feedbacks_list.first()
    latest_date = latest_feedback.created_at if latest_feedback else None
...
def parent_dashboard_view(request):
    ...
    # 6. Notes for student
    notes = Note.objects.filter(
        models.Q(standard=student_profile.standard) | 
        ...
        if "add_student" in request.POST:
            ...
            if student_form.is_valid():
                ...
                UserProfile.objects.create(
                    ...
                    standard=student_form.cleaned_data.get('standard')
                )
...
def teacher_assignment_submissions(request, assignment_id):
    ...
    return render(request, 'accounts/teacher_assignment_submissions.html', {
...
def student_assignment_detail_view(request, assignment_id):
    ...
    # Access Control
    if assignment.standard != profile.standard:
    ...
    return render(request, 'accounts/student_assignment_detail.html', {
        'assignment': assignment,
